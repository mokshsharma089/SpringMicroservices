...
import com.MS.B.controller.ApplicationAFeign;
...
public class Bservice {
	...
	public String getCustomerCarrierUsingFeign(String name) {
		if(brepo.existsById(name)==false) {
			return "No such Customer record Exists";
		}
		else {
			Optional<Bobj> obj=brepo.findById(name);
			String number =obj.get().getMobileNo();
			System.out.println("----------Feign called---------");
			ResponseEntity<String> entity =aFeign.randomName(number);
			String mobileCarrier = entity.getBody();
			return mobileCarrier;
		}
